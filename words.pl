...
@words = grep { (length($_) >= 4 or $_ eq 'bi') and !(/^(rap[ei]|arab|bitch|black|cunt|chink|dike|dyke|fag|gook|jap|jew|kike|kraut|lame|mick|nigg|pimp|retard|spic|wetback|wop|stein|berg|kline)/i or /'s$/i or /^[A-Z]+$/) } @dict;
